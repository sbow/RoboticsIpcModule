...
|---------|-----------|----------------|-----------------|
| [config/profiles/jetson_prod.toml](../config/profiles/jetson_prod.toml) | SHM (all 7 peers) | `shm:/rim_router` | `/dev/shm/rim_router_*` + sideband `/dev/shm/rim_vision_*` / `rim_ml_*` |
| [config/profiles/jetson_mixed.toml](../config/profiles/jetson_mixed.toml) | **Mixed** — SHM compute + UDS subscribers ([Phase H](../robotics-ipc-module/plans/H-mixed-transport-router.md)) | `shm:/rim_router` + `listen_uds = uds:/tmp/rim_router.sock` | SHM `/dev/shm/rim_router_{sensor,controller,vision_capture,ml_inference}` + UDS `/tmp/rim_router_{recorder,python_tooling,dashboard}.sock` |
| [config/profiles/x86_dev.toml](../config/profiles/x86_dev.toml) | UDS (all 7 peers) | `uds:/tmp/rim_router.sock` | `/tmp/rim_router_*.sock` |
...
**Transport reachability.** Node's stdlib `dgram` module supports only UDP. The F3 gateway works out of the box against `hil.toml` and `sim_cloud.toml` (UDP profiles). For `x86_dev.toml` (UDS) the gateway needs either the [`unix-dgram`](https://www.npmjs.com/package/unix-dgram) npm package or a small profile variant that swaps peer 8 to UDP. For an all-SHM `jetson_prod.toml` the gateway is not directly reachable; the practical fix now is `jetson_mixed.toml` ([Phase H](../robotics-ipc-module/plans/H-mixed-transport-router.md) / [ADR 0014](adr/0014-mixed-transport-router.md)), where the mixed router serves `dashboard_feed` (peer 8) over **UDS** from the same process that runs the SHM compute hot path — a `unix-dgram` gateway then subscribes without an SHM addon or a second host. **Do not** embed Node in `libipc` — header-only C++ keeps the boundary clean ([ADR 0004](adr/0004-robotics-module-boundaries.md)).
...
| Profile templates + `deployment-profiles.md` | [docs/deployment-profiles.md](deployment-profiles.md) — Phase F F1 + F2 + closed C5 Scopes A + B + Phase G + Phase H (5 profiles × 7 peers; routes fan out to up to `kMaxRouteDests = 8`; optional `[[topics]]` registry; optional per-topic `topic` selector on `[[routes]]` — [ADR 0013](adr/0013-per-topic-routing.md); single-host mixed transport via `MixedRouterServer` + `jetson_mixed.toml` — [ADR 0014](adr/0014-mixed-transport-router.md); only cross-host federation remains open, cross-referenced to parked C11 Option 1b) |
