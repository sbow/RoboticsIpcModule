...
| Priority-aware QoS | **Implemented (C7, 2026-06-03)** — drop-lowest-first admission control on the SHM link (`set_priority_drop_floor` / `RIM_PRIORITY_DROP_FLOOR`), per-priority drop attribution; see [ADR 0016](adr/0016-rt-hardening-and-priority-qos.md). Datagram links unaffected (no router-owned queue) |
| RT pinning / `mlockall` | **Implemented (C7, 2026-06-03)** — opt-in `router_lock_memory()` / `router_pin_to_core()` hooks (`RIM_MLOCK` / `RIM_CPU`); `SCHED_FIFO` via systemd `LimitRTPRIO=`. See [ADR 0016](adr/0016-rt-hardening-and-priority-qos.md) |
