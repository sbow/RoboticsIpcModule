...
> Code was vendored from `sbow/cpp_tricks` (`cpp_tricks/ipc/...`) as the starting point and is now evolved here. Original include paths are preserved inside `ipc/src/` so includes like `"router/foo.hpp"` and `"ipc/foo.hpp"` continue to resolve unchanged. Runtime resource names (SHM regions, UDS sockets) use the `rim_` prefix (RoboticsIpcModule) — see [scripts/README.md](scripts/README.md) for the rename rationale.
...
- Phase D3 closed: stress/soak shell scripts under `robotics-ipc-module/scripts/` + `make test-soak / test-leak-check / test-idle-cpu / test-latency-histogram`; leak check globs `/dev/shm/rim_*` + `/tmp/rim_*.sock` around the full unit+integration+router pass; idle-CPU gate at ≤ 5 % (ADR 0007 regression)
