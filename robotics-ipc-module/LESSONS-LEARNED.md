...
| `make -j` for tests is a footgun when binaries share well-known SHM / UDS paths | The first `make ci` attempt declared the gate as `ci: all test-ipc-unit test-ipc-integration test-router test-leak-check`. Under `make -j$(nproc) ci`, make sees five independent phony targets and runs them concurrently. `test-router` and `test-leak-check` (which itself runs `test-router` internally) clobbered each other's `/dev/shm/cpp_tricks_router_*` regions and the SHM scenario reported 0 frames received. Inside `test-ipc-integration`, `profile_switch_test` / `router_restart_test` / `fault_injection_test` all bind the canonical `jetson_prod.toml` SHM names and raced the same way. | Build can parallelise; tests that touch shared filesystem paths can't. Split build vs. run: phony `build-ipc-unit` / `build-ipc-integration` aggregates collect the binary deps (parallel-safe), then the test aggregates iterate sub-makes one at a time (`@$(MAKE) test-X; @$(MAKE) test-Y; ...`) so even `-jN` from the parent ends up serial at the run boundary. The `ci` recipe applies the same pattern at the top level. Local benchmark on the dev box: 32 s end-to-end. Worth carrying forward into any future test aggregate that exercises SHM or UDS. |
| `ccache` via PATH shadowing keeps the Makefile clean | The natural urge is to teach the build system about ccache (`CXX = ccache g++`, conditional `if WITH_CCACHE`, etc.). On Ubuntu the `ccache` package installs `/usr/lib/ccache/g++ → /usr/bin/ccache` automatically; making that directory the first entry on `$PATH` (via `echo "/usr/lib/ccache" >> $GITHUB_PATH` in CI, or `export PATH=/usr/lib/ccache:$PATH` locally) gets a transparent cache with **zero** Makefile change. | This is also the right shape for any forward-compat compiler wrapper (sccache, distcc, mold linker shims). Keep the build system honest about what it actually invokes (`g++`); install the indirection out-of-band where the developer / CI runner already lives. |
