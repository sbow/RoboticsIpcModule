...
| Header | Purpose |
|--------|---------|
| `router/sideband.hpp` | `SidebandRegion` POD, `SidebandHeader` (16 B in-region header, magic `'RSB1'`), version + class constants per [ADR 0005](../docs/adr/0005-payload-policy-and-sideband.md). `SidebandMemoryClass` enum (`shm` / `cuda_managed` / `cuda_host` / `nvbufsurface`) + parse/name/is-gpu helpers per [ADR 0012](../docs/adr/0012-sideband-memory-class.md) (F5) — an opaque tag the core records, never a CUDA dependency. |
| `router/topology_loader.hpp` | TOML 1.0 loader (`load_topology_from_toml_string` / `load_topology_from_toml_file`) → owning `LoadedTopology` (returns a `RouterTopology` view + `RouteRule[]` + per-peer `SidebandRegion[]` + optional `TopicEntry[]`). Requires the vendored toml++ header. |
| `router/topic_table.hpp` | Declarative topic registry (C5 Scope B). `TopicEntry { id, name, payload_class, sideband_idx }` populated from optional `[[topics]]` blocks; `topic_by_id` / `topic_by_name` lookup helpers. The router never reads the `TopicEntry` catalog on the hot path; bridges / recorders / dashboards use it to validate published frames, and the loader reads it at **load time** to validate per-topic `[[routes]]` selectors (Phase G / [ADR 0013](../docs/adr/0013-per-topic-routing.md)). Empty when the profile omits `[[topics]]`. |
| `router/routing.hpp` | Routing primitives. `RouteRule { source, dest_count, topic_id, dest[kMaxRouteDests=8] }`; `make_route` (source-only) + `make_topic_route` (per-topic, Phase G) constexpr factories; `route_targets_for(rules, count, source, topic_id=kRouteTopicAny)` — the first-match-wins hot-path lookup, now two-dimensional on `(source, topic_id)` per [ADR 0013](../docs/adr/0013-per-topic-routing.md). A rule with `topic_id == kRouteTopicAny` (the default) matches any topic = legacy source-only behavior. |
| `router/mixed_router_server.hpp` | **`MixedRouterServer`** (Phase H, [ADR 0014](../docs/adr/0014-mixed-transport-router.md)) — non-templated router for profiles whose peers span >1 transport. Holds `optional<ShmRouterLink>` + `optional<DatagramRouterLink<Uds>>` + `optional<DatagramRouterLink<Udp>>` (only the present ones), a `peer_id → transport` egress map, and a single cooperative non-blocking poll loop. Receives on each link via `try_receive`, resolves targets once with `route_targets_for`, dispatches each target to the owning link's `send_to_peer` — cross-transport delivery in one hop. `topology_is_mixed(topo)` is the selector; aggregate `forwarded()` / `dropped_full()` roll up the present links. The templated `RouterServer<T>` path is untouched (opt-in). |
...
| `recv_empty` | **Not driven yet.** Field is present for parity with `ShmRouterMetrics`. The blocking datagram `recv()` throws `runtime_error` on `SO_RCVTIMEO`; the outer `RouterServer::run` loop catches it. Phase H added the non-throwing `Udp::try_recv` / `Uds::try_recv` (`MSG_DONTWAIT`) that `MixedRouterServer` polls with, but `recv_empty` is still not driven (the mixed loop tracks idleness across all links, not per datagram link). |
...
make test-topic-dispatch    # Phase G — 1 sensor, 3 topics, 3 subscribers each see only their topic (ADR 0013)
make test-mixed-transport   # Phase H — MixedRouterServer SHM->UDS->SHM cross-transport chain (ADR 0014)
...
| GPU memory mapping (`cudaImportExternalMemory`, NvBufSurface) | The IPC core records `memory_class` but links no CUDA / NvBuffer SDK | By design ([ADR 0004](../docs/adr/0004-robotics-module-boundaries.md) / [ADR 0012](../docs/adr/0012-sideband-memory-class.md)) — the mapping code lives in the bridge / consumer process, not the header-only core. (`memory_class` parsing itself is **done** — F5 / ADR 0012.) |
...
- [examples/bridges/README.md](../examples/bridges/README.md) — Phase E E3 scaffolding for Phase F bridges (`python_peer` [implemented in F2], `node_gateway` [implemented in F3], `mavlink_gateway` [interface + ADR 0011 in F4], `vision_peer` [interface + `memory_class` parser + ADR 0012 in F5])
...
- [examples/bridges/mavlink_gateway/](../examples/bridges/mavlink_gateway/) — Phase F F4 interface + ADR stub; documents the peer-6 contract surface (compacted MAVLink → 32 B inline payload, schema per `msgid`), command-flow (controller → bridge → MCU via UART write), profile delta when the binary lands, sideband fallback for `mavlink_bulk`, systemd serial-device permissions; cross-links [ADR 0011](../docs/adr/0011-device-bridge-transports.md) for the transport rationale
- [examples/bridges/vision_peer/](../examples/bridges/vision_peer/) — Phase F F5 interface + realized `memory_class` parser; documents the peer-4 (vision) + peer-5 (ML) contract surface (metadata in the 32 B inline payload, pixels/tensors in a sideband region), the `memory_class` → access-path table (`shm` → `mmap`, `cuda_managed` → managed pointer, `cuda_host` → pinned host, `nvbufsurface` → DMA-buf import), CSI vs V4L2 capture front-ends, and the dev/prod profile split (same bridge, `shm` on x86 vs `nvbufsurface` / `cuda_managed` on Jetson); cross-links [ADR 0012](../docs/adr/0012-sideband-memory-class.md). Capture + CUDA mapping binaries are deferred (they link SDKs the core excludes)
...
- [docs/adr/0011-device-bridge-transports.md](../docs/adr/0011-device-bridge-transports.md) — transport-selection rationale for hardware-device bridges: UART/MAVLink → UDS `SOCK_DGRAM` (Phase F F4 worked example), SPI → SHM SPSC ring, I²C → UDS `SOCK_DGRAM`, CAN / CAN-FD → UDS `SOCK_DGRAM`. Addresses the `SOCK_STREAM`-vs-`SOCK_DGRAM` question head-on with alternatives considered and rejected; locks the bridge interface so downstream users have a stable contract to build against without the router learning device protocols
- [docs/adr/0012-sideband-memory-class.md](../docs/adr/0012-sideband-memory-class.md) — realizes the ADR 0008 `memory_class` forward declaration (Phase F F5). Adds `memory_class` (`shm` / `cuda_managed` / `cuda_host` / `nvbufsurface`) + optional `cuda_device` to `[[peers.sideband]]`, parsed + validated in the topology loader and surfaced on `SidebandRegion`. Defines the consumer access path each class implies. The core records the tag and links no CUDA / NvBuffer SDK — only the bridge / consumer process that actually maps GPU memory does. Default `shm` keeps every pre-F5 profile byte-identical
- [docs/deployment-profiles.md](../docs/deployment-profiles.md) — Phase F F1 operator-facing companion to [`config/profiles/`](../config/profiles/); profile selector + per-profile shape (jetson_prod / x86_dev / hil / sim_cloud) + route topology + optional `[[topics]]` schema + single-transport-per-router limitation cross-referenced to parked C11; the 2-destination cap and dashboard limitation are now historical (closed 2026-05-28 by C5 Scopes A + B); resource-name conventions + operator hand-off checklist
- [docs/adr/0013-per-topic-routing.md](../docs/adr/0013-per-topic-routing.md) — promotes `topic_id` to a dispatch key (Phase G). `RouteRule` gains an optional `topic_id` selector (`kRouteTopicAny` sentinel = match-any); `route_targets_for` dispatches first-match-wins on `(source, topic_id)`; the `[[routes]]` TOML schema gains an optional `topic` field validated against `[[topics]]`. Wire format unchanged; backward compatible (a route without `topic` keeps source-only semantics). `make_topic_route` is the constexpr factory sibling of `make_route`
- [robotics-ipc-module/plans/G-declarative-routing.md](../robotics-ipc-module/plans/G-declarative-routing.md) — **delivered 2026-06-01** Phase G (per-topic dispatch, [ADR 0013](../docs/adr/0013-per-topic-routing.md)); promoted 2026-05-30 from C5 Scope C because the per-topic routing change is phase-shaped, not close-out work (new ADR + `RouteRule` surgery + `x86_dev.toml` demo + `routing_test` / `topology_loader_test` / `topic_dispatch_test` cases). Priority-aware QoS (former C5 Scope D) is **not** part of Phase G; it was merged into [parked C7](../robotics-ipc-module/plans/post-phases-robotics-review.md#c7--real-time--production-knobs-mlockall-cpu-pinning-sched_fifo-priority-aware-qos) alongside RT pinning / `mlockall` (same latency-under-contention domain)
- [docs/adr/0014-mixed-transport-router.md](../docs/adr/0014-mixed-transport-router.md) — single-host mixed-transport routing (Phase H). `MixedRouterServer` holds one link per present transport and fans frames across SHM + UDS + UDP from one process in one hop; `forward()` split into `try_receive` + public `send_to_peer`; non-blocking `Udp`/`Uds::try_recv` for the cooperative poll; multi-listen `[router]` schema (`listen_uds` / `listen_udp`) + per-transport listen validation. Templated single-transport path untouched (opt-in). Cross-host federation parked (C11 Option 1b)
- [robotics-ipc-module/plans/H-mixed-transport-router.md](../robotics-ipc-module/plans/H-mixed-transport-router.md) — **delivered 2026-06-02** Phase H ([ADR 0014](../docs/adr/0014-mixed-transport-router.md)); promotion of parked C11's single-host half. `mixed_transport_test` (SHM→UDS→SHM chain) + multi-listen `topology_loader_test` cases + `config/profiles/jetson_mixed.toml`
