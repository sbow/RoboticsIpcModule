...
| `router/topic_table.hpp` | Declarative topic registry (C5 Scope B). `TopicEntry { id, name, payload_class, sideband_idx }` populated from optional `[[topics]]` blocks; `topic_by_id` / `topic_by_name` lookup helpers. Router never consults the registry at forward time — bridges / recorders / dashboards use it to validate published frames. Empty when the profile omits `[[topics]]`. |
...
- [robotics-ipc-module/plans/G-declarative-routing.md](../robotics-ipc-module/plans/G-declarative-routing.md) — **planned** Phase G (per-topic dispatch); promoted 2026-05-30 from C5 Scope C because the per-topic routing change is phase-shaped, not close-out work (new ADR + `RouteRule` surgery + all profiles + integration tests rebuilt). Priority-aware QoS (former C5 Scope D) is **not** part of Phase G; it was merged into [parked C7](../robotics-ipc-module/plans/post-phases-robotics-review.md#c7--real-time--production-knobs-mlockall-cpu-pinning-sched_fifo-priority-aware-qos) alongside RT pinning / `mlockall` (same latency-under-contention domain)
